...
export default function CompanyTasksTab() {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {error && (
        <div className="rounded border border-red-900 bg-red-950/50 px-3 py-2 text-sm text-red-300">{error}</div>
      )}
      ...
      <div className="flex flex-col gap-2">
        {tasks.map((task) => (
          <div key={task.id} className="flex items-center gap-3 rounded border border-neutral-800 bg-neutral-950 px-3 py-2">
            ...
            {task.assignee && <span className="shrink-0 text-xs text-neutral-500">{task.assignee}</span>}
          </div>
        ))}
      </div>
    </div>
